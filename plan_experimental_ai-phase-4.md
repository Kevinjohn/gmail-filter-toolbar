...
**Part E: Update default values in error handler (around line 125-131):**

Add the default value for `showAiNotetakersButton` in the error handler:

```javascript
// Inside the error handler block:
showAiNotetakersButton = false;  // ADD THIS LINE
```

**Part F: Update `loadState()` function (around line 57-84):**
...
**Part C: Set localized text (after existing i18n calls, around line 88):**
...
**Part B: Find the `chrome.storage.onChanged` listener and add handling:**
...
## Step 4.8: Add Localization

**File:** `src/_locales/en/messages.json`

Add these entries (keep alphabetical order):
...
```json
{
  ...
  "options_show_ai_notetakers": {
    "message": "Show AI & Transcription button",
    "description": "Checkbox label in options page to enable/disable AI & Transcription filter button."
  }
}
```

**Note:** Only English locale is required for experimental features. Other locales will fall back to English.

---

## Step 4.9: Update Background Script Defaults

**File:** `src/modules/background.js`

Update the `chrome.runtime.onInstalled` listener to include default value for AI & Transcription feature:

**Part A: Add import at top:**

```javascript
import { SHOW_AI_NOTETAKERS_KEY } from './constants.js';
```

**Part B: Update storage.sync.set call (around line 16):**
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  chrome.storage.sync.set(
    {
      gmailCalMode: 'ALL',
      [SHOW_AI_NOTETAKERS_KEY]: false,  // ADD THIS LINE
    },
    () => {
...
Update existing test file: `tests/filter.test.js`

Add test cases for `isAiNotetakerRow()` (following existing test pattern in the file):
...
```bash
git add src/modules/constants.js src/modules/state.js src/modules/filter.js src/modules/toolbar.js src/modules/background.js src/modules/options.js src/options.html src/_locales/en/messages.json src/contentScript.js tests/filter.test.js
git commit -m "feat: add experimental AI & Transcription filter

- Add AI_NOTETAKERS mode with sender name detection
- Add patterns for Gemini, ChatGPT, Claude, Copilot, Otter.ai, Fathom, Fireflies.ai
- Add experimental options toggle (defaults to off)
- Add toolbar button with smart_toy icon
- Add unit tests for isAiNotetakerRow()
- Add real-time visibility toggle from options page
- Set default value in background.js on install"
```
...
To add more AI/transcription services:
