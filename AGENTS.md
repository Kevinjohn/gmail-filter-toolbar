...
Use `pnpm run build` to produce both Chrome and Firefox bundles in `dist/chrome/` and `dist/firefox/`. Use `pnpm run build:chrome` or `pnpm run build:firefox` to build a single browser. Run `pnpm test` for the Jest unit suite with preconfigured JSDOM shims. Execute `pnpm run e2e` for Playwright flows that exercise extension UI; install browsers once using `pnpm exec playwright install`. Lint code with `pnpm run lint`, and normalize formatting through `pnpm run format` before committing. When loading the extension in browsers, use `dist/chrome/` for Chrome/Edge and `dist/firefox/` for Firefox.
